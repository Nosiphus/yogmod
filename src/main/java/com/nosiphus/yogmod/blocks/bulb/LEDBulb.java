...
public class LEDBulb extends BulbBase {

    private static final Map<World, List<LEDBulb.Toggle>> toggles = new java.util.WeakHashMap<World, List<Toggle>>(); // FORGE - fix vanilla MC-101233
    ...
    private boolean isBurnedOut(World worldIn, BlockPos pos, boolean turnOff)
    {
		...
		List<LEDBulb.Toggle> list = (List<Toggle>)toggles.get(worldIn);

        if (turnOff)
        {
            list.add(new LEDBulb.Toggle(pos, worldIn.getTotalWorldTime()));
        }
        ...
        {
            LEDBulb.Toggle blockredstonetorch$toggle = list.get(j);
	...
	public LEDBulb(boolean isOn, String name) {
