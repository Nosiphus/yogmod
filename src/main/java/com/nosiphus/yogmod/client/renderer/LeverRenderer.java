...
public class LeverRenderer implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int posX, int posY, int posZ, Block block, int modelId, RenderBlocks renderer) {
        if (modelId == renderId && block == ModBlock.LEVER || block == ModBlock.CLASSIC_LEVER) {
            ...
            if (!flag1)
            {
                renderer.setOverrideBlockTexture(renderer.getBlockIcon(ModBlock.ASPHALT));
            }
            ...
            renderer.renderStandardBlock(world.getBlock(posX, posY, posZ), posX, posY, posZ);
            ...
            tessellator.setBrightness(world.getBlock(posX, posY, posZ).getMixedBrightnessForBlock(world, posX, posY, posZ));
            tessellator.setColorOpaque_F(1.0F, 1.0F, 1.0F);
            IIcon iicon = renderer.getBlockIconFromSide(world.getBlock(posX, posY, posZ), 0);
            ...
            for (int j1 = 0; j1 < 8; ++j1)
            {
                ...
                if (i1 > 0 && i1 < 5)
                {
                    ...
                    avec3[j1].xCoord += (double)posX + 0.5D;
                    avec3[j1].yCoord += (double)((float)posY + 0.5F);
                    avec3[j1].zCoord += (double)posZ + 0.5D;
                }
                else if (i1 != 0 && i1 != 7)
                {
                    avec3[j1].xCoord += (double)posX + 0.5D;
                    avec3[j1].yCoord += (double)((float)posY + 0.125F);
                    avec3[j1].zCoord += (double)posZ + 0.5D;
                }
                else
                {
                    avec3[j1].xCoord += (double)posX + 0.5D;
                    avec3[j1].yCoord += (double)((float)posY + 0.875F);
                    avec3[j1].zCoord += (double)posZ + 0.5D;
                }
            }
