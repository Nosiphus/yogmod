...
public enum RedstoneProvider implements IBlockComponentProvider {
    ...
    public void appendBody(ITooltip tooltip, IBlockAccessor accessor, IPluginConfig config) {
        ...
        } else if (config.getBoolean(Options.REDSTONE_LEVEL) && accessor.getBlock() == ModBlocks.WIRE.get()) {
            int power = accessor.getBlockState().getValue(BlockStateProperties.POWER);
            tooltip.setLine(Options.REDSTONE_LEVEL, new PairComponent(Component.translatable("tooltip.waila.power"), Component.literal(String.valueOf(power))));
        }
    }
}